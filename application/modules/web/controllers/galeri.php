...
class galeri extends CI_Controller {
	...
	function index($uri=0)
	{
		...
		$this->breadcrumb->append_crumb('Berita',  base_url());
	...
	function album($param=0,$uri)
	{
		...
		$d['data_retrieve'] = $this->app_global_web_model->generate_index_galeri($param,$GLOBALS['site_limit_medium'],$uri);
		
		
 		$this->load->view($GLOBALS['site_theme'].'/web/bg_header',$d);
 		$this->load->view($GLOBALS['site_theme'].'/web/bg_left');
 		$this->load->view($GLOBALS['site_theme'].'/web/berita/bg_detail');
 		$this->load->view($GLOBALS['site_theme'].'/web/bg_right');
 		$this->load->view($GLOBALS['site_theme'].'/web/bg_footer');
	}
}
