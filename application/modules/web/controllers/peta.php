...
class peta extends CI_Controller {
	...
	function index($uri=0)
	{
		...
		$d['dt'] = $this->db->select('*')->join("dlmbg_kelurahan","dlmbg_kelurahan.id_kelurahan=dlmbg_objek_wisata.id_kelurahan")->get("dlmbg_objek_wisata");
		
		
 		$this->load->view($GLOBALS['site_theme'].'/web/bg_header',$d);
 		$this->load->view($GLOBALS['site_theme'].'/web/peta/bg_home');
