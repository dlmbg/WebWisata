...
class restoran extends CI_Controller {
	...
	function index($uri=0)
	{
		...
		$this->breadcrumb->append_crumb('Restoran',  base_url());
		...
		$d['data_retrieve'] = $this->app_global_web_model->generate_index_restoran($GLOBALS['site_limit_small'],$uri);
		
		
 		$this->load->view($GLOBALS['site_theme'].'/web/bg_header',$d);
 		$this->load->view($GLOBALS['site_theme'].'/web/bg_left');
 		$this->load->view($GLOBALS['site_theme'].'/web/restoran/bg_home');
