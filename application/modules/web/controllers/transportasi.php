...
class transportasi extends CI_Controller {
	...
	function index($uri=0)
	{
		...
		$d['data_retrieve'] = $this->app_global_web_model->generate_index_transportasi($GLOBALS['site_limit_small'],$uri);
		
		
 		$this->load->view($GLOBALS['site_theme'].'/web/bg_header',$d);
 		$this->load->view($GLOBALS['site_theme'].'/web/bg_left');
 		$this->load->view($GLOBALS['site_theme'].'/web/transportasi/bg_home');
