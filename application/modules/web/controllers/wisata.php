...
class wisata extends CI_Controller {
	...
	function set($jenis="",$uri=0)
	{
		...
		$this->breadcrumb->append_crumb('Kategori',  base_url());
		$this->breadcrumb->append_crumb($jenis,'/');
		...
		$d['data_retrieve'] = $this->app_global_web_model->generate_kategori_wisata($jenis,$GLOBALS['site_limit_small'],$uri);
		
		
 		$this->load->view($GLOBALS['site_theme'].'/web/bg_header',$d);
 		$this->load->view($GLOBALS['site_theme'].'/web/bg_left');
 		$this->load->view($GLOBALS['site_theme'].'/web/kategori/bg_home');
